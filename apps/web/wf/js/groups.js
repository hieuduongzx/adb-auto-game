...
// Quick-connect menu shown when an output wire is dropped on empty canvas.
// It turns the most common "drop, add a block, wire it" sequence into one
// action while keeping normal drops onto an existing node unchanged.
// `call` is intentionally absent: a Function node is only valid with a chosen
// function, which is what the Function submenu below is for — a bare call node
// from here would be an empty, unrunnable block.
const WF_QUICK_CONNECT = {
  out:   ["wait", "tap", "if_image", "log"],
  true:  ["end", "if_image", "wait", "log"],
  false: ["wait", "wait_image", "log", "end"],
  body:  ["wait", "tap", "if_image", "break"],
  done:  ["end", "log", "wait"],
  found: ["end", "log", "wait"],
  fail:  ["log", "wait", "end"],
};
...
function wfQuickConnectItems(port){
  ...
  return ids.filter((id,i)=>WF_NODES[id]&&!ids.slice(0,i).includes(id)).map(type=>({
    type, label:WF_NODES[type].label, ico:WF_NODES[type].ico,
  }));
}
...
// Human phrasing for the port a wire comes from — "out"/"true" are internal
// names; a person choosing the next block reads "Next step" / "On success".
const WF_QC_PORT = { out:"Next step", "true":"On success", "false":"On fail",
...
// ── Header dropdown menus (run cluster · device tray) ───────────────────────
// Same #wf-ctxmenu infrastructure as the canvas menus, but anchored under a
// toolbar button instead of the pointer. Clicking the same button again closes.
function wfMenuToggleCheck(btn, owner){
  const m=$("wf-ctxmenu");
  if(m && m.style.display==="block" && m.__owner===owner){ wfHideMenu(); return true; }
  return false;
}
function wfMenuShowUnder(m, btn, owner){
  m.__owner=owner;
  // Measure off-screen first so long labels can widen the card before we pin it.
  m.style.visibility="hidden"; m.style.display="block"; m.style.left="0px"; m.style.top="0px";
  const mw=m.offsetWidth, r=btn.getBoundingClientRect();
...
// ▾ next to Test block: the less-frequent run/debug/package actions.
function wfShowRunMenu(btn){
...
// ⋯ in the device tray: low-frequency recovery actions (kept out of the header
// so the two circular-arrow icons stop competing with Refresh).
function wfShowDeviceMenu(btn){
