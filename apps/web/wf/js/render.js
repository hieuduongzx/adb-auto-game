...
function wfPalPairEl(pair, catKey){
  ...
  wrap.className="wf-pal-pair cat-"+catKey;
  wrap.title=pair.hint||"";
  // Label only — the longer pairing hint stays in the tooltip (wrap.title)
  // so the tray reads as one quiet unit instead of a paragraph.
  const head=document.createElement("div"); head.className="wf-pal-pair-hd";
  head.innerHTML=`<span class="wf-pal-pair-lbl">${escHtml(pair.label||"Pair")}</span>`;
  wrap.appendChild(head);
  const row=document.createElement("div"); row.className="wf-pal-pair-row";
  (pair.types||[]).forEach((type,i)=>{
    if(i>0){
  ...
  WF_CATS.forEach(cat=>{
    ...
    if(q){
      types=types.filter(t=>{
        if(pairTypes.has(t)){
          const pair=wfPalPairByType(t);
          return pair && pair.types.some(pt=>wfPalTypeMatches(pt,q));
        }
        return wfPalTypeMatches(t,q);
      });
    }
    ...
    types.forEach(type=>{
      const pair=pairTypes.has(type) ? wfPalPairByType(type) : null;
      if(pair){
        if(seenPair.has(pair.id)) return;
        seenPair.add(pair.id);
        // Only emit the pair if at least one of its types is still in this
        // category's list (handles odd filtered states).
        if(!pair.types.some(t=>types.includes(t))) return;
        grid.appendChild(wfPalPairEl(pair, cat.key));
        return;
      }
      grid.appendChild(wfChip(WF_NODES[type].ico, WF_NODES[type].label, type, cat.key));
    });
