/* Macro2k icon set — Lucide (ISC), 24x24 grid, stroke set by icons.css.

   THE ONLY COPY OF ANY ICON GEOMETRY IN THE SUITE. Every path below is copied
   verbatim from Lucide; nothing here is drawn by hand. Do not inline an <svg>
   with its own paths in an app — call uiIco() or use a data-ico placeholder, and
   add the name here if it is missing. The old pattern was 159 inline SVGs across
   14 files at 15 stroke widths and 15 render sizes, which is what made the set
   read as a mismatched pile.

   `packaging/check_icons.py` holds that line, and runs in the build. The Hub and
   the Runner are migrated and must stay at zero inline <svg>; the Designer and
   DevScope still carry verbatim Lucide copies inline, and the checker ratchets
   those down file by file rather than pretending they are already done.

   Usage from JS that builds markup:   uiIco("save")            -> "<svg ...>"
                                       uiIco("save", "uico-3")  -> larger step
   Usage from HTML:                    <i data-ico="save"></i>
   (hydrated on DOMContentLoaded; call uiIcoHydrate(root) after injecting HTML)

   Sizes and stroke weights live in shared/icons.css. Do not set them here.
*/
(function () {
  "use strict";
  ...
  // Returns the full <svg> markup for *name*. Unknown names render nothing
  // visible rather than a mystery glyph, and say so in the console once.
  var warned = {};
  function uiIco(name, cls) {
    var body = ICO[name];
    if (!body) {
      if (!warned[name]) { warned[name] = 1; try { console.warn("uiIco: unknown icon", name); } catch (e) {} }
      body = "";
    }
    return '<svg class="uico' + (cls ?" " + cls : "") + '" viewBox="0 0 24 24" ' +
           'aria-hidden="true" focusable="false">' + body + "</svg>";
  }

  // Replace every <i data-ico="name"> / <span data-ico> placeholder under *root*
  // with real markup. Safe to call repeatedly; hydrated nodes are marked.
  function uiIcoHydrate(root) {
    var list = (root || document).querySelectorAll("[data-ico]:not([data-ico-done])");
    for (var i = 0; i < list.length; i++) {
      var el = list[i], name = el.getAttribute("data-ico");
      el.setAttribute("data-ico-done", "");
      el.innerHTML = uiIco(name, el.getAttribute("data-ico-size") || "");
    }
  }

  window.uiIco = uiIco;
  window.uiIcoHydrate = uiIcoHydrate;
  window.uiIcoNames = function () { return Object.keys(ICO); };

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", function () { uiIcoHydrate(); });
  } else {
    uiIcoHydrate();
  }
})();
